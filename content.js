    ...
    _checkOverlap: function (cA, cB) {
      for (var i = 0; i < cA.schedules.length; i++) {
        ...
        for (var j = 0; j < cB.schedules.length; j++) {
          ...
          var commonDay = schA.days.some(function (d) { return schB.days.indexOf(d) !== -1; });
          if (commonDay && schA.timeStr !== 'TBA' && schB.timeStr !== 'TBA') {
            if (schA.timeStr === schB.timeStr) return true;
          }
        }
      }
      return false;
